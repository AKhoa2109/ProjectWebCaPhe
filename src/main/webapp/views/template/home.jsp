		...
		<div class="slider">
			<div class="slides">
				<c:forEach var="slide" items="${listSlide}">
					<div class="slide">
						<img src="<%=request.getContextPath()%>/views/assets/images/Slide/${slide.anhSlide}" alt="${slide.tenSlide}" />
					</div>
				</c:forEach>
			</div>
			<!-- Nút điều hướng -->
	...
	<div class="category-container">
		...
		<div class="grid-container">
			<c:forEach var="list" items="${listLoaiSP}">
				<div class="grid-item">
					<a href="DanhMucSanPham?id=${list.maLoaiSP}"> <img src="<%=request.getContextPath()%>/views/assets/images/HinhSanPham/${list.hinhLoaiSP}" alt="${list.tenLoaiSP}">
						<p>${list.tenLoaiSP}</p>
					</a>
				</div>
			</c:forEach>
	...
	<div class="catalog-product-container">
		...
		<section class="product-grid">
			<c:forEach var="product" items="${listSP}" varStatus="status">
				<a href="ChiTietSanPhamServlet?id=${product.maSP}&type=${product.maLoaiSP}">
					<article class="product-card ${status.index >= 5 ? 'hidden' : ''}">
						<img
							src="<%=request.getContextPath()%>/views/assets/images/HinhSanPham/${product.anhSP}"
							alt="${product.tenSP}">
						<div class="product-info">
